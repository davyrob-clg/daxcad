...
static int currentbitmap; /* for uses by the environment saver */

void SetProperty(Window Win);
void SetLocalDrawable(void);
void InterceptExposures(XEvent *EventStruct);
int WindowToBitmap(Window Win, GprBitmapDesc *Desc);
void TOOLPEN();
void GPR_$SET_RASTER_OP();
void GPR_$SET_TEXT_VALUE();
void GPR_$SET_TEXT_BACKGROUND_VALUE();
void GPR_$SET_FILL_VALUE();
void GPR_$SET_FILL_BACKGROUND_VALUE();
void GPR_$SET_BITMAP();
void GPR_$SET_CLIPPING_ACTIVE();
void GPR_$PIXEL_BLT();
void GPR_$INQ_BITMAP_DIMENSIONS();
void GPR_$COLLECT_EXPOSURES();
int ValidDesc(int Desc);
void RGB(int Value, int *R, int *G, int *B);
void DAXCADFault(int Sig, int Code);
...
void GPR_$SET_BANNER(Desc, String, Length, st)
...
void GPRXError(Func, Error, Code)
...
void GPR_$SET_BITMAP_WINDOW(Desc, Win, st)
...
void GPR_$COLLECT_EXPOSURES(Count, st)
...
void GPR_$INQ_RASTER_OP(Ops, st)
...
void GPR_$SET_AUTO_UPDATE(Auto, st)
...
void GPR_$FORCE_HIDDEN_UPDATE(st)
...
void GPR_$GET_SYSTEM_COLORMAP(Pixels, NoPixels, st)
...
int *st;
{
    ...
    int ret;
    unsigned long planemask[256];
    unsigned long pixels_ul[256];
    ...
    *st = 0;
    ...
    ret = XAllocColorCells(Xdisplay, SystemColormap, True, planemask, 0, pixels_ul, *NoPixels);
...
void GPR_$WRITE_BITMAP(Bitmap, File, Length, st)
...
void GPR_$READ_BITMAP(Bitmap, Dest_x, Dest_y, File, Length, st)
